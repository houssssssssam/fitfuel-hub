...
import { MessageCircle, Send, Bot, Copy, Trash2, ChevronDown } from "lucide-react";
...
export default function ChatBot() {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      localStorage.setItem(getChatHistoryKey(), JSON.stringify(messages.slice(-20)));
    }
  }, [messages]);
  ...
  const sendMessage = async (content: string) => {
    ...
    const userMessage: Message = { role: "user", content, type: "text", foodLogged: null };
  ...
  return (
    <div ref={chatRef} className="fixed bottom-4 sm:bottom-6 right-4 sm:right-6 z-50 flex flex-col items-end pointer-events-none">
      <div
        className={`bg-background border border-border shadow-2xl rounded-2xl overflow-hidden transition-all duration-300 origin-bottom-right flex flex-col pointer-events-auto ${
          isOpen ? "w-[95vw] sm:w-[380px] h-[75vh] sm:h-[520px] scale-100 opacity-100 mb-4" : "w-[380px] h-[520px] scale-50 opacity-0 absolute bottom-16 right-0"
        }`}
      >
          ...
          >
            ...
            <Button
              ...
              className={`shrink-0 h-9 w-9 rounded-xl mb-0.5 ml-1 transition-all flex items-center justify-center font-bold relative ${
                input.trim() ? "bg-green-600 hover:bg-green-500 text-white shadow-md shadow-green-900/20" : "bg-white/5 text-muted-foreground"
              }`}
            >
              <Send className={`h-4 w-4 ${input.trim() ? "translate-x-[1px] translate-y-[-1px]" : ""}`} />
            </Button>
          </form>
          {input.length > 200 && (
